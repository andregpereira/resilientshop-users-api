...
public record UsuarioRegistroDto(
		...
		@NotBlank(message = "Insira o CPF") @Size(message = "O CPF deve ter 11 números") @CPF(message = "CPF inválido ou inexistente. Formatos aceitos: xxx.xxx.xxx-xx, xxxxxxxxx-xx ou xxxxxxxxxxx") String cpf,
		@Size(message = "O telefone deve ter no máximo 20 caracteres", max = 20) @Pattern(message = "Formato do telefone inválido. Por favor, informe um telefone no seguinte formato +xxx (xx) xxxxx-xxxx", regexp = "^[+]\\d{2,3} [(]\\d{2}[)] \\d{4,5}[-]\\d{4}$") String telefone,
